...
public class xXGamSolicitudCrearCO extends OAControllerImpl {
   ...
   public static final String RCS_ID = "$Header: xXGamSolicitudCrearCO.java 1.1 2013/02/27 10:51:48 eroncoroni ship $";
   public static final boolean RCS_ID_RECORDED = VersionInfo.recordClassVersion("$Header: xXGamSolicitudCrearCO.java 1.1 2013/02/27 10:51:48 eroncoroni ship $", "%packagename%");
   ...
   public void processRequest(OAPageContext pageContext, OAWebBean webBean) {
      ...
      pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Version 1.1  - processRequest", 3);
      pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Initial Variable", 3);
     ...
     String lAccountDefault = "0";
      ...
      xXGamInvSoliVOImpl invSoliVOImplClear = (xXGamInvSoliVOImpl)oaapplicationmodule.findViewObject("xXGamInvSoliVO1");
      xXGamInvSoliDtlVOImpl invSoliDtlVOpImplClear = (xXGamInvSoliDtlVOImpl)oaapplicationmodule.findViewObject("xXGamInvSoliDtlVO1");
      invSoliVOImplClear.clearCache();
      invSoliDtlVOpImplClear.clearCache();
      pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Inicializo las VO", 3);
      invSoliVOImplClear.initQuery("-1");
      invSoliDtlVOpImplClear.initQuery("-1");
      pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Crea Solicitud", 3);
      pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - buscamos el KIT - sqlKi por User_idt", 3);
//      OracleCallableStatement pstmtKit = (OracleCallableStatement)oadbtransactionimpl.createCallableStatement("SELECT NVL(XIK.KIT_ID,-1) , XUK.KIT_CODE , FLV.MEANING FROM XXGAM_HR_UNIFORMS_KIT_V XUK , XXGAM_INV_KIT XIK , FND_LOOKUP_VALUES FLV , FND_LOOKUP_VALUES FLV2 , FND_USER FU WHERE 1 = 1 AND XUK.KIT_CODE = XIK.KIT_COD(+) AND FLV.LOOKUP_TYPE = \'GAM_HR_UNIFORMES\' AND XUK.KIT_CODE = FLV.LOOKUP_CODE AND XUK.LANGUAGE = FLV.LANGUAGE AND FLV2.LOOKUP_TYPE = \'XXGAM_APERTURA_DE_PERIODOS\' AND FLV.LOOKUP_CODE = FLV2.MEANING AND FLV.LANGUAGE = FLV2.LANGUAGE AND (FLV2.START_DATE_ACTIVE IS NULL OR FLV2.START_DATE_ACTIVE <= TRUNC(SYSDATE)) AND (FLV2.END_DATE_ACTIVE IS NULL OR FLV2.END_DATE_ACTIVE >= TRUNC(SYSDATE)) AND XUK.LANGUAGE = \'ESA\' AND XUK.PERSON_ID = FU.EMPLOYEE_ID AND FU.USER_ID = ?", 1);
      ...
      try {
        ...
        if(rsK.next()) {
            ...
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - KIT: " + lKitCode, 3);
         }else{
            System.out.println("...No se encontraron datos que leer en ejecucion de query con lUserId: " + lUserId);
         }
         ...
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sqlKit Error: " + var83.getMessage().toString(), 3);
      } finally {
         try {
            pstmtKit.close();
         } catch (Exception var79) {
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sqlKit Error: " + var79.getMessage().toString(), 3);
       ...
       try {
          ...
          callablePeriodos.setString(1, am.obtieneEmployeeNumber(oadbtransactionimpl.getUserId()).toString());
          callablePeriodos.setInt(2, Integer.parseInt(am.obtienePersonId(pageContext.getUserId()).toString()));
          ResultSet resultQuery = callablePeriodos.executeQuery();

          if(resultQuery.next()) {
              lsolicitudAnterior = resultQuery.getString(1); 
              System.out.println("Solicitud anterior: " + lsolicitudAnterior);
          }else{
             System.out.println("...No se encontraron solicitudes anteriores");
          }
       } catch (SQLException var83) {
          pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sql get request to Employee Error: " + var83.getMessage().toString(), 3);
          System.out.println("Error  SQLException " + var83.getMessage().toString());
       } catch(Exception e){
           pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sql get request to Employee Global Error: " + e.getMessage().toString(), 3);
           System.out.println("Error  SQLException " + e.getMessage().toString());
       }finally {
          try {
             callablePeriodos.close();
          } catch (Exception var79) {
            System.out.println("Error Exception: " + var79.getMessage().toString());
             pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sql get request to Employee Error: " + var79.getMessage().toString(), 3);
             throw OAException.wrapperException(var79);
          }
       }
       /**
         ...
         * */
         ...
         OracleCallableStatement pstmAccounDefault = (OracleCallableStatement)oadbtransactionimpl.createCallableStatement(" SELECT NVL (default_code_comb_id, 0) " + 
                                                                                                                          ...
                                                                                                                          "   AND person_id = ?", 1);
          try {
             pstmAccounDefault.setInt(1, Integer.parseInt(am.getPersonId(pageContext.getUserId()) + ""));
      ...
      if (!v_estatusEmployee.toString().equals("1") && !v_estatusEmployee.toString().equals("NA")){
          ...
          String lmensage = am.getMessageError(pageContext, webBean, Integer.parseInt(am.obtienePersonId(pageContext.getUserId()).toString())); 
      ...
      } else if((lPeriodValid.equals("N")) && !("UNICO".equals(lsolicitudAnterior))){
            if (lPeriodDetail.equals("NPOST")){
                lMessageError = " Error a\u00fan no se encuentra abierto tu periodo de registro de Uniformes, comprende del " + lFechaInicial + " al " + lFechaFinal + ".";      
                lMessageError = " Error a\u00fan no se encuentra abierto tu periodo de registro de Uniformes, comprende del " + lFechaInicial + " al " + lFechaFinal + ".";      
            }else{
                lMessageError = " Error en el periodo de solicitud de Uniformes, el kit " + lKitCode    + " ha vencido, comprende del " + lFechaInicial + " al "+ lFechaFinal + ".";
            }
      ...
      /*
       ...
       throw new OAException("FND", "FND_GENERIC_MESSAGE", tokens);         
         ...
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Buscamos los datos del empleado sqlNombreEmpleado", 3);
         ...
         try {
            ...
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Crea Nro de Solicitud: " + soliD.trim().toString(), 3);
         } catch (SQLException var81) {
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sqlNombreEmpleado Error: " + var81.getMessage().toString(), 3);
         } finally {
            try {
               pstmtSoli.close();
            } catch (Exception var80) {
               pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sqlNombreEmpleado Error: " + var80.getMessage().toString(), 3);
               throw OAException.wrapperException(var80);
            }
         }

         boolean lFlagLoop = false;
         xXGamInvSoliDtlVOImpl invSoliDtlVOImpl = (xXGamInvSoliDtlVOImpl)oaapplicationmodule.findViewObject("xXGamInvSoliDtlVO1");
         invSoliDtlVOImpl.initQuery(lSoliId.toString());
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Buscamos las dotaciones configuradas-sqlDota", 3);
         ...
         try {
            ...
            while(rsD.next()) {
               ...
               invSoliDtlVOImpl.getCurrentRow().setAttribute("SoliId", lSoliId);
               invSoliDtlVOImpl.getCurrentRow().setAttribute("SoliDtlId", Long.valueOf(rsD.getLong(1)));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("DotaId", Long.valueOf(rsD.getLong(2)));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("UniformTypeCod", rsD.getString(3));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("Nomenclature", rsD.getString(4));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("MeasureUnitCod", rsD.getString(5));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("CycleCod", rsD.getString(6));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("QtyPlanta", Long.valueOf(rsD.getLong(7)));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("QtyEventual", Long.valueOf(rsD.getLong(8)));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("SustitucionSwitcher", rsD.getString(9));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("Status", rsD.getString(10));
               invSoliDtlVOImpl.getCurrentRow().setAttribute("Attribute1", lAdscr);
               if("Planta".equals(lAdscr)) {
                  invSoliDtlVOImpl.getCurrentRow().setAttribute("Attribute2", rsD.getString(7));
               } else {
                  invSoliDtlVOImpl.getCurrentRow().setAttribute("Attribute2", rsD.getString(8));
               }

               lFlagLoop = true;
               pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Crea Dotacion Detalle Id : " + rsD.getString(1), 3);
            }
            ...
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sqlDota Error: " + var85.getMessage().toString(), 3);
         } finally {
            try {
               pstmtDota.close();
            } catch (Exception var78) {
               pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - sqlDota Error: " + var78.getMessage().toString(), 3);
   ...
   public void processFormRequest(OAPageContext pageContext, OAWebBean webBean) {
      ...
      if("grabar".equals(lEvent)) {
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Event Grabar", 3);
         ...
         oadbtransactionimpl.commit();
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - ejecuta XXGAM_INV_MOVEORDER_PKG.VALIDA_SUSTITUTOS", 3);
         ...
         try {
            ...
            pstmtSust.execute();
            pstmtSust.close();
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - ejecutado con exito", 3);
         } catch (SQLException var33) {
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - pstmtSust Error: " + var33.getMessage().toString(), 3);

            try {
               pstmtSust.close();
            } catch (Exception var32) {
               pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - pstmtSust Error: " + var32.getMessage().toString(), 3);
            }
         ...
         oadbtransactionimpl.commit();
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Commit Fin Grabar", 3);
         xXGamInvSoliDtlVOImpl invSoliDtlVOImpl1 = (xXGamInvSoliDtlVOImpl)oaapplicationmodule.findViewObject("xXGamInvSoliDtlVO1");
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Reconsulto la VO para el Soli Id" + lSoliIdC.toString(), 3);
      ...
      if("generarpedido".equals(lEvent)) {
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Event generarpedido", 3);
         ...
         if("CERRADO".equals(lStatus) || "PREAPROBADO".equals(lStatus)) {
            ...
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Error: " + lMessageE, 3);
            MessageToken[] tokens1 = new MessageToken[]{new MessageToken("MESSAGE", lMessageE)};
            throw new OAException("FND", "FND_GENERIC_MESSAGE", tokens1);
         }
         ...
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Ejecuta concurrente para SOLI ID " + lSoliIdSTR, 3);
         ...
         if(lReqId.compareTo(0) <= 0) {
            ...
            pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - Error al ejecutar concurrente", 3);
            MessageToken[] tokens3 = new MessageToken[]{new MessageToken("MESSAGE", lMessageE1)};
            throw new OAException("FND", "FND_GENERIC_MESSAGE", tokens3);
         }

         MessageToken[] tokens2 = new MessageToken[]{new MessageToken("MESSAGE", "Se ejecuto la creaci\u00f3n del pedido verifique el concurrente REQUEST ID: " + lReqId.toString())};
         pageContext.writeDiagnostics(this, "xXGamSolicitudCrearCO - verifique el concurrente REQUEST ID: " + lReqId.toString(), 3);
         OAException message1 = new OAException("FND", "FND_GENERIC_MESSAGE", tokens2, (byte)3, (Exception[])null);
